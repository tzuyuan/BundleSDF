...
class FeatureArray(nn.Module):
    ...
    def __init__(self, num_frames, num_channels):
        ...
        self.data = nn.parameter.Parameter(
            torch.normal(0, 1, size=[num_frames, num_channels]).float(),
            requires_grad=True,
        )
...
class PoseArray(nn.Module):
    ...
    def __init__(self, num_frames, max_trans, max_rot):
        ...
        self.data = nn.parameter.Parameter(
            torch.zeros([num_frames, 6]).float(), requires_grad=True
        )
        self.register_parameter("data", self.data)
    ...
    def get_matrices(self, ids):
        ...
        Ts = (
            torch.eye(4, device=self.data.device)
            .reshape(1, 4, 4)
            .repeat(len(ids), 1, 1)
        )
        mask = ids != 0
...
def get_embedder(multires, cfg, i=0, octree_m=None):
    if i == -1:
        ...
        embed = GridEncoder(
            ...
            desired_resolution=cfg["finest_res"],
            base_resolution=cfg["base_res"],
            level_dim=cfg["feature_grid_dim"],
        )
        print(embed)
        out_dim = embed.out_dim
    elif i == 2:
...
class NeRFSmall(nn.Module):
    def __init__(
        ...
        torch.nn.init.constant_(
            self.sigma_net[-1].bias, 0.1
        )  # Encourage last layer predict positive SDF
...
def get_sdf_loss(
    ...
    front_mask, sdf_mask, fs_weight, sdf_weight = get_masks(
        z_vals, target_d, truncation, cfg, dir_norm=dir_norm
    )
    ...
    fs_loss = (
        torch.mean(((predicted_sdf - cfg["fs_sdf"]) * mask) ** 2 * sample_weights)
        * fs_weight
    )

    mask = (
        front_mask & (target_d <= cfg["far"] * cfg["sc_factor"]) & (predicted_sdf < 1)
    )
    empty_loss = (
        torch.mean(torch.abs(predicted_sdf - 1) * mask * sample_weights)
        * cfg["empty_weight"]
    )
    fs_loss += empty_loss
    ...
    sdf_loss = (
        torch.mean(
            ((z_vals + predicted_sdf * truncation) * sdf_mask - target_d * sdf_mask)
            ** 2
            * sample_weights
        )
        * sdf_weight
...
def ray_box_intersection_batch(origins, dirs, bounds):
    ...
    tmin = (
        torch.gather(bounds[..., 0], dim=1, index=sign[:, 0].reshape(-1, 1)).reshape(-1)
        - origins[:, 0]
    ) * inv_dirs[
        :, 0
    ]  # (N)
    ...
    tmax = (
        torch.gather(
            bounds[..., 0], dim=1, index=1 - sign[:, 0].reshape(-1, 1)
        ).reshape(-1)
        - origins[:, 0]
    ) * inv_dirs[:, 0]
    tymin = (
        torch.gather(bounds[..., 1], dim=1, index=sign[:, 1].reshape(-1, 1)).reshape(-1)
        - origins[:, 1]
    ) * inv_dirs[:, 1]
    tymin[tymin < 0] = 0
    tymax = (
        torch.gather(
            bounds[..., 1], dim=1, index=1 - sign[:, 1].reshape(-1, 1)
        ).reshape(-1)
        - origins[:, 1]
    ) * inv_dirs[:, 1]
    ...
    tzmin = (
        torch.gather(bounds[..., 2], dim=1, index=sign[:, 2].reshape(-1, 1)).reshape(-1)
        - origins[:, 2]
    ) * inv_dirs[:, 2]
    tzmin[tzmin < 0] = 0
    tzmax = (
        torch.gather(
            bounds[..., 2], dim=1, index=1 - sign[:, 2].reshape(-1, 1)
        ).reshape(-1)
        - origins[:, 2]
    ) * inv_dirs[:, 2]
